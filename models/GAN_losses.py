...
'''
Taken (with a few modifications) from https://github.com/S-aiueo32/sr-pytorch-lightning/blob/8969bbd2869873e4c9e0e2c30eb5bfb0884ebf48/models/losses.py
'''
...
class VGGLoss(nn.Module):
    ...
    """
    def __init__(self, net_type='vgg19', layer='relu5_4', rescale=0.006, num_channels=3):
...
class GradLoss(nn.Module):
    """
    Take gradient of generated and traget images and calculate the loss.
    """
    ...
    def get_grads(self, image):
        gradients = torch.gradient(image, dim = (2,3))
        avg_gradients = torch.sqrt(gradients[0]**2 + gradients[1]**2)
        return avg_gradients

    def forward(self, predictions, targets):
        pred_grads = self.get_grads(predictions)
        target_grads = self.get_grads(targets)
        return self.loss(pred_grads, target_grads)